...
function wpgeoip_redirect()
{
	...
	if((strpos($ip, ":") === false)) {
		$gi = \crivion\geoip_open(plugin_dir_path(__FILE__). 'GeoIP.dat', GEOIP_STANDARD);
		$countryCode = \crivion\geoip_country_code_by_addr($gi, $ip);
	}
	else {
		$gi = \crivion\geoip_open(plugin_dir_path(__FILE__). 'GeoIPv6.dat',GEOIP_STANDARD);
		$countryCode = \crivion\geoip_country_code_by_addr_v6($gi, $ip);
	}
